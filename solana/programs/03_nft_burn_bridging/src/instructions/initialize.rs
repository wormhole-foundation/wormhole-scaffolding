...
use mpl_token_metadata::{accounts::Metadata, ID as METADATA_ID};
...
pub struct Initialize<'info> {
    ...
    seeds = [Metadata::PREFIX, &METADATA_ID.to_bytes(), &collection_mint.key().to_bytes()],
    bump,
    seeds::program = METADATA_ID,
    has_one = update_authority,
  )]
    pub collection_meta: Account<'info, MetadataAccount>,
